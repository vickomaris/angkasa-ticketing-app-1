...
const BookingCard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://dead-rose-train.cyclic.app/v1/admin/booking/user/${user_id}`
      )
      .then((response) => {
  ...
  return (
    <>
      ...
      {data == null ? (
        <>
          <div>
            <p className="h1">You have no ordering data.</p>
          </div>
        </>
      ) : (
        <>
          {data.map((item, index) => (
            <div key={index} className={styles.CardBooking}>
              ...
              <div className="d-flex flex-row align-items-center ">
                ...
                <div className={`col-md-3 ${styles.statuspayment}`}>
                  {item.payment_status === true ? (
                    <p className={styles.setStatus}> Succes </p>
                  ) : (
                    <p className={styles.setStatus2}> Waiting for payment </p>
                  )}
                </div>
